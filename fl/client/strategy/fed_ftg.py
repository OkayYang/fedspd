...
class FedFTG(BaseClient):
    """
    FedFTG客户端实现 - 用于Fine-Tuning Global Model via Data-Free Knowledge Distillation
    ...
    def local_train(self, sync_round: int, weights=None):
        ...
        # 4. 返回更新后的权重给服务器，同时返回样本数和平均损失以及标签计数
        avg_loss = total_loss / (len(self.train_loader) * self.epochs)
        return model_weights, num_sample, avg_loss, dict(label_counter)